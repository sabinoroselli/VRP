    ...
    if checking_feasibility == sat:
        ...
        arrivals = [
            [
                ...
                +
                jobs[elem1.split('_')[0] + '_' + elem1.split('_')[1]]['tasks'][elem1.split('_')[2]]['Service']
        ...
        service_per_node = []
        for index,route in enumerate(routes):
            service_buffer = [0 for _ in list(new_paths[index].values())[0]]
            service_buffer[0] = jobs[
                route[0].split('_')[0] + '_' + route[0].split('_')[1]
                ]['tasks'][route[0].split('_')[2]]['Service']
            service_buffer[-1] = jobs[
                route[1].split('_')[0] + '_' + route[1].split('_')[1]
                ]['tasks'][route[1].split('_')[2]]['Service']
            for ind, pair in enumerate(list(new_paths[index].values())[1:]):
                intermediate_service_buffer = [0 for _ in pair[1:]]
                intermediate_service_buffer[-1] = jobs[route[ind + 2].split('_')[0] + '_' + route[ind + 2].split('_')[1]
                                                  ]['tasks'][route[ind + 2].split('_')[2]]['Service']
                service_buffer += intermediate_service_buffer
            service_per_node.append(service_buffer)

        ######### end of weird manipulation #############
        ...
        routes_plus = [[
            ...
            (actual_nodes[index],tw_per_node[index],service_per_node[index]),
            current_routes[index][5],
            current_routes[index][6]
            ]for index,route in enumerate(routes)
        ]
