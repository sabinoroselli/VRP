...
def json_parser(file_to_parse,monolithic = False):
    ...
    if monolithic == False:
        return jobs,nodes,edges,Autonomy,ATRs,charging_coefficient,start_list
    else:
...
def paths_formatter(current_routes,current_paths,tasks):
    ...
    return shortest_paths_solution, paths_combo, paths_combo_edges

def paths_formatter_2(current_routes,current_paths,tasks):
    # this dictonary contains the paths used to traverse the routes
    paths_combo = {
        route: {
            (tasks[first], tasks[second]):
                current_paths[(first, second)]
            for first, second in zip(current_routes[route][4][:-1], current_routes[route][4][1:])
        }
...
def make_Sequence_Planner_json(node_sequence, current_routes):
    the_dict = {'robots': []}
    for robot1 in current_routes:
        path = []
        for path_elem_index1, path_elem1 in enumerate(current_routes[robot1][0]):
            pair_buffer = []
            for robot2 in current_routes:
                for path_elem_index2, path_elem2 in enumerate(current_routes[robot2][0]):
                    if (robot2, path_elem2) in [(i[0], node_sequence[i][0]) for i in node_sequence] \
                            and robot2 != robot1 \
                            and path_elem2 == path_elem1 \
                            and int(str(node_sequence[(robot2, path_elem_index2)][1])) \
                            < \
                            int(str(node_sequence[(robot1, path_elem_index1)][1])):
                        pair_buffer.append((robot2, path_elem_index2))
                ...
                path.append({
                    ...
                    'pre': [{
                        "name":[i for i in pair_buffer
                            if int(str(node_sequence[i][1])) == max([int(str(node_sequence[i][1]))
    ...
    with open('Sequence_planner_translator.json', 'w+') as write_file:
