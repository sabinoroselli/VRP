...
def routing(edges, jobs, tasks, Autonomy, start_list, ATRs, current_path, previous_routes = []):
    ...
    # binary variable that keeps track of changed routes from previous iterations
    prev_change = m.addVars(len(previous_routes),max_routes,vtype=GRB.BINARY,name='prev_change')
    # bla bla ...to be defined
    buffer_bla = m.addVars(len(previous_routes),max_routes,max_routes,vtype=GRB.BINARY,name='buffer_bla')
    ...
    ############ TO USE WHEN SOLVING INSTANCE 2547_25_60_9 ###################
    # prova = m.addConstr(
    #     quicksum([direct_travel[i,j,k]
    #                 for i in range(max_routes)
    #                 for k in tasks
    #                 for j in tasks
    #                 if j == 'start_14_0'
    ...
    if previous_routes != []:
        ...
        # excluding_previous_routes_1 = m.addConstrs(
        #     (buffer_bla[i,r,k] == 1)
        #     >>
        #     (quicksum([direct_travel[k,elem1,elem2]
        #               for elem1,elem2 in zip(route[:-1],route[1:]) ]) <= len(route) - 1)
        #
        #     for i, iteration in enumerate(previous_routes)
        #     for r,route in enumerate(iteration)
        #     for k in range(max_routes)
        # )
        for i, iteration in enumerate(previous_routes):
            # print('iteration',i)
            for r, route in enumerate(iteration):
                ...
                for k in range(max_routes):
                    m.addConstr(
                        ...
                        >>
                        (quicksum([direct_travel[k, elem1, elem2]
                                   for elem1, elem2 in zip(route[:-1], route[1:])]) <= len(route) - 2)
                    )
        # excluding_previous_routes_2 = m.addConstrs(
        #     (prev_change[i,r] == 1)
        #     >>
        #     quicksum([ buffer_bla[i,r,k] for k in range(max_routes) ]) == max_routes
        #     for i, iteration in enumerate(previous_routes)
        #     for r, route in enumerate(iteration)
        # )
        ...
        # excluding_previous_routes_3 = m.addConstrs(
        #     quicksum([ prev_change[i,r] for r,_ in enumerate(iteration) ]) >= 1
        #     for i, iteration in enumerate(previous_routes)
        # )
